...
class ManualDispatchFrontendStaticContractTest(unittest.TestCase):
    ...
    def test_order_details_reuses_form_grid_in_read_only_mode(self):
        ...
        self.assertIn("function createOrderReadOnlyField", order_modal_renderer)
        self.assertIn("const fieldCard = createDetailField", order_modal_renderer)
        self.assertIn("function createOrderReadOnlySelect", order_modal_renderer)
        self.assertIn("return createDetailField(label, selected ? selected.label : formState[field])", order_modal_renderer)
    ...
    def test_phase5_opshop_pickup_detail_modal_is_read_only(self):
        ...
        self.assertIn('setButtonContent(closeButton, "Close", "x", { iconAfter: true })', modal_renderer)
    ...
    def test_phase7_opshop_assignment_frontend_contract(self):
        ...
        self.assertIn("const canGenerate = hasAssignedTasks && !hasLockedFinalSummary && !hasSavedFinalSummary", trip_summary_renderer)
        self.assertIn("const deliveryGenerateButton = canGenerate && assignedOpShopPickups.length === 0", trip_summary_renderer)
        self.assertIn("const opshopGenerateButton = canGenerate && assignedOpShopPickups.length > 0", trip_summary_renderer)
        self.assertIn("createDeliveryOrdersPanel", trip_summary_renderer)
        self.assertIn("assigned-delivery-orders-section", trip_summary_renderer)
        self.assertIn("if (!hasOrderTasks && !hasSavedFinalSummary)", trip_summary_renderer)
    ...
    def test_phase8_opshop_pickup_list_frontend_contract(self):
        ...
        date_group_renderer = (
            FRONTEND_ROOT / "js" / "render" / "opshop-date-group-list-renderer.js"
        ).read_text(encoding="utf-8")
        ...
        self.assertIn("createOpShopDateGroupList", modal_renderer)
        self.assertIn("groupOpShopPickupsByDate", date_group_renderer)
        ...
        self.assertIn("if (!leftDriverName && rightDriverName)", modal_renderer)
        self.assertIn("formatDateHeading", date_group_renderer)
        self.assertIn("Monday", date_group_renderer)
        self.assertIn("opshop-date-group", date_group_renderer)
        self.assertIn("opshop-date-group-toggle", date_group_renderer)
        self.assertIn('toggle.setAttribute("aria-expanded", String(!collapsed))', date_group_renderer)
        self.assertIn('toggle.setAttribute("aria-controls", listId)', date_group_renderer)
        self.assertIn("state.collapsedRegularOpShopPickupDates", modal_renderer)
        self.assertIn("getDateGroupCollapsed", date_group_renderer)
        self.assertIn("list.hidden = collapsed", date_group_renderer)
        self.assertIn("toggle.addEventListener(\"click\", (event) =>", date_group_renderer)
        self.assertIn("event.preventDefault()", date_group_renderer)
        self.assertIn("event.stopPropagation()", date_group_renderer)
        self.assertNotIn("if (event.target !== backdrop)", modal_renderer)
        self.assertNotIn("backdrop.addEventListener(\"click\"", modal_renderer)
        self.assertIn("Collapsed", date_group_renderer)
        self.assertIn("Expanded", date_group_renderer)
        ...
        self.assertIn('setButtonContent(editButton, "Edit", "pencil")', list_item_renderer)
    ...
    def test_oncall_opshop_pickup_frontend_contract(self):
        ...
        self.assertIn("padding: 0.68rem 0.75rem 0.86rem;", styles)
        self.assertIn("background: linear-gradient(180deg, #ffffff, #f8fbf8);", styles)
        self.assertIn("min-height: 72px;", styles)
        self.assertIn("padding: 0.9rem 1rem 1.05rem;", styles)
        self.assertIn("gap: 0.42rem;", styles)
        self.assertIn("background: linear-gradient(135deg, #0f5f46, #0a4635);", styles)
        self.assertIn(".opshop-template-picker-option *", styles)
        self.assertIn("color: #ffffff;", styles)
        self.assertIn("background: linear-gradient(135deg, #137454, #0e5c45);", styles)
        self.assertIn("background: linear-gradient(135deg, #176d51, #0d513d);", styles)
    ...
    def test_countryside_opshop_pickup_frontend_contract(self):
        ...
        self.assertIn('routeGroup.append(', modal_renderer)
        self.assertIn('createIcon("route")', modal_renderer)
        self.assertIn('document.createTextNode(`Route Group:', modal_renderer)
