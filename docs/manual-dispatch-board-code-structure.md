...
## Compatibility Facades and Module Packages

| Stable entry point | Implementation package | Responsibility |
| --- | --- | --- |
| `backend/api/manual_dispatch.py` | `backend/api/manual_dispatch_routes/` | Shared HTTP helpers plus auth, Delivery, OP SHOP, snapshot, Attache, export, and legacy route factories |
| `backend/services/manual_dispatch_service.py` | `backend/services/manual_dispatch/application/` and `backend/services/manual_dispatch/audit/` | Public service compatibility plus bounded application and audit orchestration |
| SQLite/In-Memory repository facades | `backend/repositories/sqlite/` and `backend/repositories/in_memory/` | Matching order, assignment, specification, OP SHOP, snapshot, legacy, and auth persistence mixins |
| `frontend/js/api/manual-dispatch-api.js` | `frontend/js/api/manual-dispatch/` | Stable 96-export barrel plus shared, Delivery, OP SHOP, and legacy request clients |
| `frontend/js/actions/workspace-actions.js` | `frontend/js/actions/workspace/` | Stable 78-action factory plus request context, guards, loaders, reset, and bounded workflows |
| `frontend/js/render/delivery-workspace-renderer.js` | `frontend/js/render/delivery/` | Delivery page, Task Pool, Trip Summary, Run Sheet, History, modals, and local utilities |
| `frontend/js/render/opshop-workspace-renderer.js` | `frontend/js/render/opshop/` | OP SHOP page, subtype Task Pools, Trip Summary, Collections, History, Templates, and local utilities |

Route factories register directly on the shared router and resolve the current service late, preserving route order and mutable test seams. Application services and audit recorders preserve the facade's exceptions, return values, Logbook actions, actor/date rules, and best-effort behavior. Repository mixins preserve method signatures, SQL, transactions, ordering, row mapping, and schema initialization.

Frontend child modules point toward shared helpers, never back toward their compatibility facade. The API, action, Delivery renderer, and OP SHOP renderer graphs are acyclic. `app-state.js`, its ordered 183-field shape, `selectors.js`, and `frontend/styles.css` remain unchanged.
...
- `backend/api/manual_dispatch.py`: stable router/service facade. Route factories live under `backend/api/manual_dispatch_routes/`; scoped and legacy routes remain available.
- `backend/schemas.py`: request, response, and snapshot dataclasses.
- `backend/services/manual_dispatch_service.py`: stable 93-method facade used by route handlers and tests; application services and audit recorders own extracted orchestration.
...
`sqlite_manual_dispatch_repository.py` and `in_memory_manual_dispatch_repository.py` remain stable facades with matching 108-method public contracts. Matching mixins under `backend/repositories/sqlite/` and `backend/repositories/in_memory/` own cohesive persistence responsibilities. `backend/db/schema.sql` and additive initialization in `backend/db/connection.py` keep older SQLite databases readable.
...
- `workspace-actions.js`: stable `createWorkspaceActions` facade; modules under `frontend/js/actions/workspace/` own request state, guards, loaders, and scoped Delivery/OP SHOP actions.
...
- `opshop-workspace-renderer.js`: stable facade; 10 modules under `frontend/js/render/opshop/` own Task Pool variants, Trip Summary, Templates, Collections, Saved History, page chrome, and local utilities.
...
- `delivery-workspace-renderer.js`: stable facade; 10 modules under `frontend/js/render/delivery/` own Task Pool, Trip Summary, Run Sheets, Saved History, page chrome, modals, and local utilities.
...
- `tests/test_refactor_contract_baseline.py`: exact route, Service, Repository, frontend action, state, renderer, and API facade fingerprints.
- `tests/test_manual_dispatch_frontend_static_contract.py`: module paths, API-only network calls, DOM/static safety, and existing feature contracts.
