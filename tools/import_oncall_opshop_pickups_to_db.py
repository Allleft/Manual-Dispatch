...
def main(argv=None):
    ...
    try:
        ...
        record_maintenance_event(
            action="ONCALL_WORKBOOK_IMPORT_COMPLETED",
            result="FAILED",
            workspace="OPSHOP",
            actor=actor,
            entity_type="OPSHOP_WORKBOOK_IMPORT",
            entity_id=f"oncall:{workbook_filename}",
            summary="Oncall OP SHOP workbook import failed.",
            metadata={
                "mode": "apply",
    ...
    event_summary = (
        "Oncall OP SHOP workbook import completed with unresolved driver aliases: "
        f"{summary.rows_imported} rows imported."
        if has_unresolved_aliases
        else (
            "Oncall OP SHOP workbook import completed: "
            f"{summary.rows_imported} rows imported."
        )
    )
    record_maintenance_event(
